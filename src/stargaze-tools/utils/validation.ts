...
export const checkFiles = (images: File[], metadata: File[]) => {
  ...
  const sortedImages = [...images.map(f => f.name).map(parseFileName)].sort(function (a, b) {
    return a - b;
  });
  const sortedMetadata = [...metadata.map(f => f.name).map(parseFileName)].sort(function (a, b) {
    return a - b;
  });
  let lastValue;
...
export const validateMetadata = (
  ...
  metadata.map(async (file, index: number) => {
    ...
    // else error
    metadataJson.attributes.map((item: any) => {
